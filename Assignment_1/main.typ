...
#show: ieee.with(
  title: [Assignment 1],
  abstract: [
    Using a dataset generated by ChatGPT to mimic real world consumer behavior, we want to determine the factors that would influence customers to be subscribed to a business by creating a .
  ],
  authors: (
    (
...
@ecology compares classification algorithms that are commonly found in ecological studies, namely linear discriminant analysis (LDA), logistic regression, additive logistic regression and classification trees across 3 different datasets. All 3 datasets were used to create a predicting model

This paper concludes that random forest, in principle, outperforms LDA and logistic regression when there are strong interactions among variables. The author notes 2 key features that sets the random forest algorithm apart. One is its ability to evaluate the importance of features and the possibility of imputing data using proximities, a metric on how often 2 data points end up in the same leaf node across all trees in the forest. The author did point out that random forest shouldn't be a tool for traditional statistical inference such as ANOVA or hypothesis testing.
...
We want to standardize numerical values so that

The `Subscription Status`, `Discount Applied`, and `Promo Code used` columns consists of binary values, so I'll just map it as `Yes = 1` and `No = 0`.
...
One-Hot encoding is used for the rest of the categorical data as those columns have low cardinality. Although my dataset has the gender column consists of values as male or female, I plan to use One-Hot as its encoding technique for unseen data that lists more than one gender.
